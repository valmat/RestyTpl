-- Пропаченная версия. Убрал некоторые ассерты и убрал кеширование.
-- 
-- 
...
local function new(template, safe)
    ...
    function template.precompile(view, path, strip, plain)
        local chunk = dump(template.compile(view, plain), strip ~= false)
        if path then
    ...
    function template.compile(view, plain)
        -- assert(view, "view was not provided for template.compile(view, plain)")
        return load_chunk(template.parse(view, plain)), false
    end

    function template.compile_file(view)
        return template.compile(view, false)
    end

    function template.compile_string(view)
        return template.compile(view, true)
    end
...
local function echo(...) for i=1,select("#", ...) do ___[#___+1] = tostring(select(i, ...)) end end
    ...
    function template.process(view, context, plain)
        -- assert(view, "view was not provided for template.process(view, context, plain)")
        return template.compile(view, plain)(context)
    end

    function template.process_file(view, context)
        -- assert(view, "view was not provided for template.process_file(view, context)")
        return template.compile(view, false)(context)
    end

    function template.process_string(view, context)
        -- assert(view, "view was not provided for template.process_string(view, context)")
        return template.compile(view, true)(context)
    end

    function template.render(view, context, plain)
        -- assert(view, "view was not provided for template.render(view, context, plain)")
        template.print(template.process(view, context, plain))
    end

    function template.render_file(view, context)
        -- assert(view, "view was not provided for template.render_file(view, context)")
        template.render(view, context, false)
    end

    function template.render_string(view, context)
        -- assert(view, "view was not provided for template.render_string(view, context)")
        template.render(view, context, true)
    end
